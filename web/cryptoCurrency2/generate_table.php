		...
		if($user_row['user_name'] == $_SESSION["user_name"]){
			...
			foreach ($db->query('SELECT user_id, name, price, volume FROM currency') as $currency_row){
				if($currency_row['user_id'] == $_SESSION["userID"] && $currency_row['name'] == $currency){
					$table = $table .'<form action="addInvestment.php" method="GET"><tr clas="'. $currency_row['name'] . '"><td>'. $currency_row['name']."</td><td>".$currency_row['price']."</td><td>". $currency_row['volume']."</td>". '<td><input type="submit" name="' . $currency_row['name'] . '" value="invest" onclick="investing(this)"></td><td><input type="number"> </td>' ."</tr></form>";
				}
			}
		} 
	}
...
elseif(isset($_SESSION["user_name"]) || isset($_SESSION['userID'])){
	foreach ($db->query('SELECT user_id, user_name FROM users') as $user_row){
		if($user_row['user_name'] == $_SESSION["user_name"] || $user_row['user_id'] == $_SESSION['userID']){
			...
			foreach ($db->query('SELECT user_id, name, price, volume FROM currency') as $currency_row){
				if($currency_row['user_id'] == $userID){
					$table = $table .'<tr clas="'. $currency_row['name'] . '"><td>'. $currency_row['name']."</td><td>".$currency_row['price']."</td><td>". $currency_row['volume']."</td>". '<td><input type="button" name="' . $currency_row['name'] . '" value="invest" onclick="investing(this)"></td><td><input type="number"> </td>' ."</tr>";
				}
			}
		} 
	}
